...
每个新任务在业务事务提交前先持久启动 Workflow；启动失败则回滚，已启动而业务回滚的孤立 Workflow 查不到任务后结束。2026-09-26 后仅由持久 Workflow 按该任务 ID 推进，订单请求不再通过 `after()` 建立第二条发送路径。数据库原子领取、租约及每台打印机单一活跃发送限制并发；发送尝试先落库，再调用 XPYUN。执行器有任务数和时间上限，不扫描历史队列。

XPYUN 请求固定 `mode=1`，`expiresIn` 取任务创建后 120 秒窗口的剩余秒数，云端在短暂断线时可缓冲。已取得远端 ID 的任务只查询 `queryOrderState`，不再发送。未知结果最多在窗口内使用原幂等键作一次持久计数的受控重试；第二次仍不明则停在 `unknown`，不换键、不切备用。超过窗口且确定未受理的任务停在 `expired`；已有远端 ID 的任务继续只读确认最多十分钟，仍未确认则保留 ID 并标记 `unknown`，不能推定未打印。人工仅对没有远端 ID 的 `failed` 或 `expired` 重新打印，新建 `reprint` 意图并保留原记录；不能自动处理 `unknown`。`completed` 仅表示 XPYUN 报告完成，不代表酒店现场已经出纸。
