...
Native Android client: the signed release APK is `distribution/site/public/releases/rdv-order-0.1.4.apk`, connected to the same production backend. See [delivery record](docs/deployment-delivery.md), [Android builds/tests/signing](android/README.md) and [remaining hotel acceptance](docs/android-acceptance.md). `npm run android:apk` continues to export the separate internal debug build.
